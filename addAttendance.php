...
<h1 style="margin-left:300px; text-shadow:3px 4px 6px #3498db;">CLASS : MCA(Master of computer Application)</h1>
    <table border="1" cellspacing="0" class="table table-striped" style="width:100%; text-align:center; background-color: rgb(202, 216, 226);">
...
$fetchingStudents = mysqli_query($db,"SELECT * FROM student_attendance") OR die (mysqli_error($db));
...
<tr>
    ...
    <td colspan="4" style="background-color:rgb(161, 219, 243);"><input type="date" name="selected_date"></td>
</tr>
<tr>
    <td colspan="5" style="background-color:rgb(194, 207, 207)">
    <!-- <input style="background-color: rgb(165, 192, 162); padding: 7px 20px;"
     type="submit" name="addAttendanceBTN"> -->
...
">View_Attandance_Record</a></button>
   <button><a href="addingStudents.php" style="display: inline-block;
    padding: 12px 24px;
    background-color: #3498db;
    color: #fff;
    text-decoration: none;
...
">Add Student</a></button>
   <button><a href="delete(mca).php" style="display: inline-block;
    padding: 12px 24px;
    background-color: #3498db;
    color: #fff;
    text-decoration: none;
...
$selected_date = date("Y-m-d");
    ...
    if(isset($_POST['studentPresent'])){
        ...
        foreach($studentPresent as $std){
            mysqli_query($db,"INSERT INTO attendance (student_id,curr_date,attendance_month,attendance_year,attendance) 
