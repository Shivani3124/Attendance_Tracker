...
<h1 style="margin-left:300px; text-shadow:3px 4px 6px #3498db;">CLASS : BSc(Bachlors of Science (Compurt Science))</h1>
...
$fetchingStudents = mysqli_query($db,"SELECT * FROM student_bsc") OR die (mysqli_error($db));
...
<tr>
    ...
    <input class="btn btn-primary" type="submit" value="Submit" name="addAttendanceBTN"></td>
</tr>
    </form>
    </table>
    <button><a href="attensheet(Bsc).php" style="display: inline-block;
    padding: 12px 24px;
...
">View_Attandance_Record(BscCs)</a></button>
   <button><a href="addingStudents(Bsc).php" style="display: inline-block;
    padding: 12px 24px;
    background-color: #3498db;
    color: #fff;
    text-decoration: none;
...
">Add Student</a></button>
  <button><a href="delete(bsc).php" style="display: inline-block;
    padding: 12px 24px;
    background-color: #3498db;
    color: #fff;
    text-decoration: none;
...
$selected_date = date("Y-m-d");
    ...
    if(isset($_POST['studentPresent'])){
        ...
        foreach($studentPresent as $std){
            mysqli_query($db,"INSERT INTO atten_bsc (student_id,curr_date,attendance_month,attendance_year,attendance) 
