...
$student=mysqli_query($db,"SELECT student_name FROM student_attendance");
...
<table border="1" cellspacing="0" cellpadding="5" width="100%" class="table table-striped">
    ...
    text-align: center;
    padding: 20px 0; text-shadow:2px 3px 7px black;">Attendance Tracker <br>ATTENDANCE OF MCA </h1>
           ...
           echo "<td>".$studentsNameArray[$count]."</td>";
            for($j=1;$j<=$totalDaysInMonth;$j++)
            {
                ...
                // $q="SELECT attendance FROM attendance WHERE student_id='".$studentIdArray[$count]."' AND curr_date='".$dateofAttendance."'";
                 $fetchingStudents_atten= mysqli_query($db,"SELECT attendance FROM attendance 
...
<!-- <input type="submit" value="Add Students" onsubmit="addingstudents.php"> -->
